...
function initCalendar() {
    ...
    calendar = new FullCalendar.Calendar(calendarEl, {
        ...
        locale: 'pt-br',
        height: 'auto',
        dayMaxEventRows: true, // Evita que a cela do día creza infinitamente
        headerToolbar: {
        ...
        eventContent: function(arg) {
            ...
            /**
             * AJUSTE DE UI: 
             * - Reducimos o padding (p-0.5)
             * - Fixamos o tamaño das fotos para seren máis visibles (w-6 h-6)
             * - Limitamos o texto para non forzar a altura do card
             */
            let html = `
                <div class="p-0.5 px-1 rounded shadow-sm ${bgClass} border-l-[3px] overflow-hidden w-full flex flex-col justify-center min-h-[36px]">
                    <div class="flex justify-between items-center leading-none mb-0.5">
                        <span class="text-[8px] font-black uppercase truncate opacity-70">${ext.setor}</span>
                    </div>
                    <div class="flex items-center gap-1.5 overflow-hidden">
                        <div class="flex -space-x-1.5 shrink-0">
                            ${ext.foto1 ? `<img src="${ext.foto1}" class="w-6 h-6 rounded-full border border-white bg-slate-200 object-cover shadow-sm">` : ''}
                            ${ext.foto2 ? `<img src="${ext.foto2}" class="w-6 h-6 rounded-full border border-white bg-slate-200 object-cover shadow-sm">` : ''}
                        </div>
                        <div class="text-[9px] font-bold text-slate-900 leading-[1.1] truncate">
                            ${arg.event.title}
                        </div>
                    </div>
                </div>
